...
export class BikeBookingWidgetComponent implements OnChanges {
  ...
  ngOnChanges({ bikeData }: SimpleChanges): void {
    const { currentValue: bike } = bikeData as {
      currentValue: ExpandedBikeData;
    };
  ...
  onDatesRangeSet({ startDate, endDate }: DatesRange): void {
    ...
    if (startDate && endDate) {
      if (this.isHalfDay && startDate.isSame(endDate, 'day')) {
  ...
  public onNextMonthRequest = async (month: string): Promise<void> => {
  ...
  onSelectOpen(isOpened: boolean, side: 'start' | 'end'): void {
    if (isOpened) {
      if (side === 'start') {
