...
@Component({
  ...
  providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: {
      showError: true, displayDefaultIndicatorType: false
    }
  }, {
...
export class BikesRequestFlowComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.activateRoute.data.pipe(
      map(({user, bike}) => {
        const isBike = bike && bike.queryParams && bike.queryParams.start_date;
        if (isBike) {
          const arr = [];
          const start = bike.queryParams.start_date || now;
          const duration = bike.queryParams.duration;
          start && arr.push(new Date(start));
          duration
            ? arr.push(new Date(moment.unix(moment(new Date(start)).unix() + Number(duration)).toISOString()))
        ...
        if (user && isBike) {
          ...
          if (user.street && user.zip && user.city && user.country && user.pretty_phone_number) {
