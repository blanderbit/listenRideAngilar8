...
export class MyBikesAvailabilityModalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.availabilityIds.forEach(id => this.availabilityForm.addControl(id, new FormControl({
      begin: this.data.timeSlots[id].start_date,
      end: new Date(new Date(this.data.timeSlots[id].start_date).getTime() / 1000 + (this.data.timeSlots[id].duration))
    })));
  }
