...
import {MbscRangeOptions, mobiscroll} from "@mobiscroll/angular";
...
import {RIDER_TAX, VAT_TAX} from "../../../modules/bikes-request-flow/consts/consts";
mobiscroll.settings = {
  theme: 'material',
  themeVariant: 'light'
};
...
export class BookWidgetComponent {
  ...
  setCalendarCounts(duration) {
    ...
    if (this.insuranceEnable) {
      ...
      this.result.basicCoverage = ((this.coverage_total || 0) / 1000) * real_day;
      this.result.coverageTotal = this.coverage_total;
      total += (premiumCoverage + this.result.basicCoverage);
    }

    if (includeFee) {
      this.result.serviceFee = ((this.result.forDicount * RIDER_TAX * VAT_TAX) + (this.result.forDicount * RIDER_TAX));
      total += this.result.serviceFee
    }
    total += this.result.subtotal;
    this.result.subtotalDiscounted = (((this.result.count * parseInt(this.daily)) / 100) * real_day);
  ...
  test(e: any) {
      ...
      const duration = moment(new Date(value[0])).diff(new Date(value[1]), 'seconds') * -1;
